...
func TestAppModuleBasic_GetQueryCmd(t *testing.T) {
	...
	require.Equal(t, "get-all-messages", cmd.Commands()[0].Name())
	require.Equal(t, "get-block-message-ids", cmd.Commands()[1].Name())
	require.Equal(t, "get-message", cmd.Commands()[2].Name())
	require.Equal(t, "get-num-messages", cmd.Commands()[3].Name())
}
